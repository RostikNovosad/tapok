$(document).ready(function () {
  if ((window.location.hash !== '' && $(window).scrollTop() > 0) || $(window).scrollTop() > 0) {
  ...
  $('.feedback__close').on('click', function () {
    ...
    if ($('.presentation__wrapper').css('display') === 'none' || $('.create__settings').length) {
      $('body').removeClass('modal-open');
    }
  });
  ...
  $('.tariffs__block-more').click(function () {
    ...
    $('.tariffs__block-item').not(accordionItem).find('.tariffs__block-list').slideUp(300);
    $('.tariffs__block-item')
      .not(accordionItem)
